...
def create_tide_table_if_not_exist(db_connection, tide_table_name):
    ...
    #     logging.info(f'Table "{tide_table_name}" already existed. Skipping creation')
    # else:
    #     logging.info(f'Table "{tide_table_name}" created.')
...
def create_location_table_if_not_exist(db_connection, location_table_name):
    ...
    cursor.execute(sql)
    # if cursor.rowcount <= 0:
    #     logging.info(f'Table "{location_table_name}" already existed. Skipping creation')
    # else:
    #     logging.info(f'Table "{location_table_name}" created.')
    db_connection.commit()
...
def write(db_connection, table_name: str, port_id: str, port_id_2_detail_map: dict[str, tuple], wait_interval: int):
    try:
        port_name, region_id = port_id_2_detail_map[port_id]
        tide_list = parse_record(region_id, port_id, port_name)
        if len(tide_list) == 0:
            logging.warning("No record found, something is wrong with the url?")
        else:
            logging.debug(f"inserting records, port_id={port_id}, region_id={region_id}, "
                               f"no. record={len(tide_list)} to db")

        for record in tide_list:
            insert(db_connection, table_name, record)
        db_connection.commit()
        logging.info(f"{len(tide_list)} tide records for {port_name} saved.")
        time.sleep(wait_interval)
        return True
    except (urllib.error.HTTPError, http.client.IncompleteRead) as e:
        msg = f"parsing record for {port_id} tried {parse_record.retry.statistics['attempt_number']} " \
...
def main(config_file: str, port_id: str, num_workers: int, log_level: LogLevel):
    ...
    for pid in port_id:
        # (name, region_id)
        name, region_id = port_id_2_detail_map[pid]
        args.append((region_id, pid, name))
    # # db_connection, table_name: str, port_id: str, port_id_2_detail_map: dict[str, tuple], wait_interval:
    # # func = functools.partial(write,
    # #                          db_connection=conn,
    # #                          table_name=config['DEFAULT']['Tide_table'],
    # #                          port_id_2_detail_map=port_id_2_detail_map,
